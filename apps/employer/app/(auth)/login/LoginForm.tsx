...
export function LoginForm() {
  ...
  const onSubmit = async (values: LoginSchema) => {
    ...
    if (!values.email.endsWith("@example.com")) {
      setFormError(
        "We could not sign you in with those details. Please review your credentials and try again.",
      );
    }
  };
  ...
  return (
    <AuthCard>
      <AuthHeader
        title="Sign in"
        subtitle="Use a secure sign in method to access your workspace."
        icon={employerAuthContent.accentIcon}
      />
      {formError ? (
        <AuthStatusBanner
          type="error"
      ...
      <AuthFooter>
        <p>{employerAuthContent.supportText}</p>
      </AuthFooter>
      <AuthLegalText>{employerAuthContent.legalText}</AuthLegalText>
    </AuthCard>
  );
}
