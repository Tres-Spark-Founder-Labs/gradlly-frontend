...
const loginSchema = z.object({
  email: z.email("Enter a valid provider email address."),
...
export function LoginForm() {
  ...
  const onSubmit = async () => {
    ...
    setFormError(
      "We could not sign you in right now. Please verify your credentials and retry.",
    );
  };
  ...
  return (
    <AuthCard>
      <AuthHeader
        title="Sign in"
        subtitle="Access secure provider tooling and dashboards."
        icon={providerAuthContent.accentIcon}
      />
      {formError ? (
        <AuthStatusBanner
          type="error"
      ...
      <AuthFooter>
        <p>{providerAuthContent.supportText}</p>
      </AuthFooter>
      <AuthLegalText>{providerAuthContent.legalText}</AuthLegalText>
    </AuthCard>
  );
}
